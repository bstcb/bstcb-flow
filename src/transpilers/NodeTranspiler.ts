    ...
    for (let token of this.tokens) {
      switch (token.type) {
        case NodeTokenKind.NTK_INPUT:
          ...
          console.log(token)
          CodeGen.genInput(
            VariableParser.parse(token.value!, NodeTokenKind.NTK_INPUT),
          )
          break
        case NodeTokenKind.NTK_OUTPUT:
          ...
          CodeGen.genOutput(
            VariableParser.parse(token.value!, NodeTokenKind.NTK_OUTPUT).value,
          )
          break
        case NodeTokenKind.NTK_IF_CONDITION:
          console.log('parsing IF CONDITION node')
          CodeGen.genIf(
            VariableParser.parse(token.value!, NodeTokenKind.NTK_IF_CONDITION)
              .value,
          )
          break
        case NodeTokenKind.NTK_FOR_LOOP:
          ...
          CodeGen.genFor(
            VariableParser.parse(token.value!, NodeTokenKind.NTK_FOR_LOOP)
              .value,
          )
          break
        case NodeTokenKind.NTK_WHILE_LOOP:
          ...
          CodeGen.genWhile(
            VariableParser.parse(token.value!, NodeTokenKind.NTK_WHILE_LOOP)
              .value,
          )
          break
        case NodeTokenKind.NTK_IF_CONDITION_END:
        case NodeTokenKind.NTK_FOR_LOOP_END:
        case NodeTokenKind.NTK_WHILE_LOOP_END:
          console.log('parsing BLOCK END node')
          CodeGen.genBlockEnd()
          break
